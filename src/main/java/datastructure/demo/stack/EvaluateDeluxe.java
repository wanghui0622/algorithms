...
public class EvaluateDeluxe {
    ...
    public static double evaluateDeluxe(String ss) {
        ...
        for (int i = 0; i < ss.length(); i++) {
            ...
            while (true) {
                 ...
                 * 1. 符号栈为空
                 * 2. 左括号直接入栈
                 * 3. 当前运算符高于栈顶运算符时，入栈(目的：确保优先级较高的运算符被优先计算)
                 */
                ...
                /*
                 * 计算表达式
                 * 1. 如果符号栈栈顶是个左括号，当前符号必须是右括号
                 * 2. 计算运算符和两个操作数，并将结果压入值栈
                 */
                String op = ops.pop();
                if (op.equals("(")) {
                    assert s.equals(")");
    ...
    public static void main(String[] args) {
        System.out.println(EvaluateDeluxe.evaluateDeluxe("3*(4+2)-1"));
        System.out.println(EvaluateDeluxe.evaluateDeluxe("3 + 5 * 6 - 7 * ( 8 + 5 )"));
    }
}
